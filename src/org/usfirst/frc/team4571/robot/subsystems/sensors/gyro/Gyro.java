...
public class Gyro {
    ...
    private Notifier mNotifier = new Notifier(mUpdateHandler);
    ...
    private class UpdateHandler implements Runnable {
        ...
        public void run() {
            ...
            if (GyroInterface.StatusFlag.VALID_DATA != status || !errors.isEmpty()) {
                System.out.println("Gyro read failed. Status: " + status + ". Errors: " + errors.stream().map(ErrorFlag::name).collect(Collectors.joining( ", ")));
                return;
            }
